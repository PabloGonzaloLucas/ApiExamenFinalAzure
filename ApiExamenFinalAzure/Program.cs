...
// Build once so we can pull initial KeyVault configuration values.
var tempProvider = builder.Services.BuildServiceProvider();
var secretClient = tempProvider.GetRequiredService<SecretClient>();

// Load KeyVault secrets once.
...
KeyVaultSecret secretoSecretKey = await secretClient.GetSecretAsync("secretkeyprueba");
...
// Initialize encryption using the same secrets model.
HelperCryptography.Initialize(builder.Configuration, keyVaultSecrets);

// Configure JWT using the same SecretClient (or refactor helper to accept KeyVaultAccesorModel).
HelperActionOAuthService helper = new HelperActionOAuthService(builder.Configuration, secretClient);
